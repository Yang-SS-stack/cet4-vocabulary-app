...
function VocabularyPage({ books = wordBooks, loadWords = loadWordBook, loadAudio = loadAudioManifest }) {
  ...
  const selectBook = async (book) => {
    ...
    isLeaving.current = true
    const transitionId = ++loadRequestId.current
    const audioId = ++audioRequestId.current
    ...
    if (audioRequestId.current === audioId) setAudioManifest(loadedAudio ?? null)
  }
  ...
  return <section className="vocabulary-page" aria-labelledby="vocabulary-heading">
    ...
    {pageData.words.length === 0 ? <div className="vocabulary-empty">
      ...
      <ul className="vocabulary-list" aria-label={book.wordListLabel} key={`${bookId}:${displayedQuery}:${displayedSort}:${currentPage}`}>
        {pageData.words.map((item) => <WordCard item={item} audioManifest={audioManifest} showFrequency={bookId === 'cet4-high-frequency'} key={item.word} />)}
      </ul>
